...
% inertial force, IBPG-A allows updating each matrix many times before moving 
% on to the other without restarting step; see the paper 
...
%       options.display: option for displaying the result, default=1 (yes)
% Output
%       (Wn,Hn): solution 
%        e: sequence of relative error ||X-WH||_F/||X|| with respect to iterations
%        t: sequence of running time with respect to iterations
%
% Written by Le Thi Khanh Hien
% Last update June-2020
%%
function [Wn,Hn,e,t] = IBPG_A(X,r,options) 
...
while i <= options.maxiter && t(i) < options.timemax  
    ...
    while j<=inneriterW &&  eps >= (delta)^2*eps0
       W=Wn;
       Wnold=Wn - Wold;
       Wy = Wn + ww(i)*Wnold; 
       gradWy = Wy*HHt - XHt; 
    ...
    j=1; eps0 = 0; eps = 1; 
    while j<=inneriterH &&  eps >= (delta)^2*eps0
       H=Hn;
       Hnold=Hn - Hold;
       Hy = Hn + wh(i)*Hnold;
       gradHy = WtW*Hy - WntX;  
